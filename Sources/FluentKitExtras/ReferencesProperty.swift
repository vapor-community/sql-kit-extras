        ...
        FromProp: FluentKit.QueryableProperty & _SQLKitExtrasSendableMetatype, FromProp.Model == From, FromProp.Value: Hashable
{
    ...
    /// Use this initializer when a ``ReferencesProperty`` refers to a ``PointerProperty`` on the same model which contains
    /// it; to avoid an infinite recursion in the model's initializer, the key path provided to the `pointsTo` parameter of
    /// the pointer property must be repeated here.
    ...
    /// Use this initializer when a ``ReferencesProperty`` refers to an ``OptionalPointerProperty`` on the same model which
    /// contains it; to avoid an infinite recursion in the model's initializer, the key path provided to the `pointsTo`
